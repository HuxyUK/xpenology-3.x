...
# fgrep "#define MPT_DEBUG" mpt2sas_debug.h | sed 's/#define //;s/\s\s*/ /g;s/\(MPT_DEBUG.*\) \(0x.*\)/\t\t\2 => "\1",/'
# fgrep "#define MPT_DEBUG" mpt2sas_debug.h | sed 's/#define //;s/\s\s*/ /g;s/\(MPT_DEBUG_.*\) \(0x.*\)/\t{level=>"\2",name=>"\1"},/'
my $gen_debug = {
	mptsas => [
...
{mask=>"0x00040000",name=>"MPT_DEBUG_CSMISAS"},
...
{mask=>"0x00010000",name=>"MPT_DEBUG_CSMISAS"},
...
sub do_card_menu
{
	...
	while ($option < 0) {
		...
		if (!($option =~ /^\n/) && $option == 0) {
			$selected_card = -1;
		} elsif ($option =~ /^\n/ || $option < 0 || ($option >= $index && $option < 97) || $option > 99) {
			$option = -1;
		} elsif ($option == 97) {
...
sub do_debug_menu
{
	...
	while ($option != 0) {
		...
		printf("\nToggle debug option [1-23, 99 or 0 to exit] > ");

		$option = <STDIN>;
		if ($option =~ /^\n/ || $option < 0 || ($option > 23 && $option < 99) || $option > 99) {
			$option = -1;
		} elsif ($option == 99) {
			my $tmp_level;
